...
async function getUser(admin, contract, username) {
  ...
  return await userJs.getUserByAddress(admin, address);
}

async function getUsers(admin, contract) {
  const { users: usersHashmap } = await rest.getState(admin, contract, options);
  const { values } = await getState(admin, { name: 'Hashmap', address: usersHashmap }, options);
  const addresses = values.slice(1);
  return await userJs.getUsers(admin, addresses);
}
