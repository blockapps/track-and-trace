...
describe("Track and Trace - deploy contracts", function() {
  ...
  before(async () => {
    // get ttRole Enums
    TtRole = await getEnums(
      `${process.cwd()}/${config.dappPath}/ttPermission/contracts/TtRole.sol`
    );
    ...
    assert.isDefined(
      ...
      "MASTER_TOKEN should be defined"
    );
    assert.isDefined(
      process.env.DISTRIBUTOR_TOKEN,
      "DISTRIBUTOR_TOKEN should be defined"
    );
    assert.isDefined(
      process.env.MANUFACTURER_TOKEN,
      "MANUFACTURER_TOKEN should be defined"
    );
    assert.isDefined(
      process.env.RETAILER_TOKEN,
      "RETAILER_TOKEN should be defined"
    );
    assert.isDefined(
      process.env.REGULATOR_TOKEN,
      "REGULATOR_TOKEN should be defined"
    );
  });
  ...
  it("should upload the contracts", async function() {
    ...
    const masterEmail = oauthHelper.getEmailIdFromToken(
      masterCredentials.token
    );
    console.log("Creating master", masterEmail);
    const masterResponse = await oauthHelper.createStratoUser(
      masterCredentials,
      masterEmail
    );
    assert.strictEqual(
    ...
    const distributorEmail = oauthHelper.getEmailIdFromToken(
      distributorCredentials.token
    );
    console.log("Creating distributor", distributorEmail);
    const distributorResponse = await oauthHelper.createStratoUser(
      distributorCredentials,
      distributorEmail
    );
    assert.strictEqual(
      distributorResponse.status,
      RestStatus.OK,
      distributorResponse.message
    );

    const manufacturerEmail = oauthHelper.getEmailIdFromToken(
      manufacturerCredentials.token
    );
    console.log("Creating manufacturer", manufacturerEmail);
    const manufacturerResponse = await oauthHelper.createStratoUser(
      manufacturerCredentials,
      manufacturerEmail
    );
    assert.strictEqual(
      manufacturerResponse.status,
      RestStatus.OK,
      manufacturerResponse.message
    );

    const retailerEmail = oauthHelper.getEmailIdFromToken(
      retailerCredentials.token
    );
    console.log("Creating retailer", retailerEmail);
    const retailerResponse = await oauthHelper.createStratoUser(
      retailerCredentials,
      retailerEmail
    );
    assert.strictEqual(
      ...
      RestStatus.OK,
      retailerResponse.message
    );

    const regulatorEmail = oauthHelper.getEmailIdFromToken(
      regulatorCredentials.token
    );
    console.log("Creating regulator", regulatorEmail);
    const regulatorResponse = await oauthHelper.createStratoUser(
      regulatorCredentials,
      regulatorEmail
    );
    assert.strictEqual(
      regulatorResponse.status,
      RestStatus.OK,
      regulatorResponse.message
    );
    ...
    const dapp = await dappJs.uploadContract(
      adminResponse.user,
      ttPermissionManager
    );
    ...
    const ttAdminUser = await dapp.createUser(args);
