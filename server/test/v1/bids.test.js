...
describe("Bids End-To-End Tests", function() {
  ...
  before(async function() {
    ...
    assert.isDefined(
      distributerToken.token,
      "distributer token is not defined"
    );
    ...
    const assetState = await post(
      handleEventUrl,
      { assetEvent: AssetEvent.REQUEST_BIDS },
      manufacturerToken.token
    );
    assert.equal(
      AssetState.BIDS_REQUESTED,
      assetState,
  ...
  it("Create Bid", async function() {
    ...
    const regulatorEmail = oauthHelper.getEmailIdFromToken(
      regulatorToken.token
    );
    bidDetail = await post(
      endpoints.Bids.bids,
      { address: asset.address, owner: asset.owner, bidValue, regulatorEmail },
      distributerToken.token
    );
    assert.equal(
      bidDetail.assetOwner,
      asset.owner,
      "owner address should be same"
    );
    assert.equal(
      bidDetail.asset,
      asset.address,
      "asset address should be same"
    );
  ...
  it("ACCEPT - handle event", async function() {
    ...
    const bidState = await post(
      url,
      { chainId: bidDetail.chainId, bidEvent: BidEvent.ACCEPT },
      manufacturerToken.token
    );
    assert.equal(
      bidState,
      BidState.ACCEPTED,
      "bid state should be in Accepted state"
    );
  });
  ...
  it("REJECT - handle event", async function() {
    ...
    const bidState = await post(
      url,
      { chainId: detail.chainId, bidEvent: BidEvent.REJECT },
      manufacturerToken.token
    );
    assert.equal(
      bidState,
      BidState.REJECTED,
      "bid state should be in Rejected state"
    );
  });
